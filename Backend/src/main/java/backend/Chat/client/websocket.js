...
function connect() {
    ...
    ws.onmessage = function(event) { // Called when client receives a message from the server
            ...
            // Check if the message is a response to the /getGroups command
            if (event.data.startsWith("Groups: ")) {
                var groups = event.data.substring(8).split(", ");
                log.innerHTML += "Available groups: " + groups.join(", ") + "\n";
            } else {
